 ...
 */
public class ClassMethodTreeVisitor extends TreePathScanner<Object, Trees> {
    ...
    public Object visitClass(ClassTree classTree, Trees trees) {
        ...
        if (trees.getElement(path) != null){
            ...
            ClassInfoDataSetter.populateClassInfo(clazzInfo, classTree,
                    path, trees);
            clazzInfo.setIdClass(ASTAnalyzerController.getClassIdCouter());
            System.out.println("#" + clazzInfo.getFlatName());
        }
    ...
    public Object visitMethod(MethodTree methodTree, Trees trees) {
        ...
        if (trees.getElement(methodPath) != null){
            ...
            MethodInfoDataSetter.populateMethodInfo(methodInfo, clazzInfo, methodTree, methodPath, trees);
            methodInfo.setIdMethod(ASTAnalyzerController.getMethodIdCouter());
            System.out.print(" +" + methodInfo.getOwningClass().getName() + "." + methodInfo.getName() + "(");
            int nParams = 0;
            for (Map.Entry<String, TypeInfo> param : methodInfo.getParameters().entrySet()) {
                System.out.print(param.getValue().toString());
                if (nParams < methodInfo.getParameters().size() - 1) {
                    System.out.print(",");
                }
                nParams++;
            }
